...
func TestRunTask(t *testing.T) {
	...
	mockECS.EXPECT().RunTask(gomock.Any()).Do(func(input interface{}) {
		...
		if int64(count) != aws.Int64Value(req.Count) {
			t.Errorf("count should be [%v]. Got [%v]", count, aws.Int64Value(req.Count))
		}
...
func TestDescribeService(t *testing.T) {
	...
	for i := 0; i < 9; i++ {
		mockECS.EXPECT().DescribeServices(gomock.Any()).Return(nil, errors.New("describe-services error"))
	}
	_, err := client.DescribeService("")
	if err == nil {
		t.Errorf("Expected DescribeService to error with \"describe-services error\"")
	}
	...
	serviceDetail := &ecs.Service{
		ServiceArn: serviceIDs[0],
	}

	mockECS.EXPECT().DescribeServices(gomock.Any()).Do(func(input interface{}) {
		// verify input fields
		req := input.(*ecs.DescribeServicesInput)
		if clusterName != *req.Cluster {
			t.Errorf("Expected request.cluster to be [%s] but got [%s]",
				clusterName, *req.Cluster)
		}
		if len(serviceIDs) != len(req.Services) || *serviceIDs[0] != *req.Services[0] {
			t.Errorf("Expected request.services to be [%v] but got [%v]", *serviceIDs[0], *req.Services[0])
		}
	}).Return(&ecs.DescribeServicesOutput{Services: []*ecs.Service{serviceDetail}}, nil)
